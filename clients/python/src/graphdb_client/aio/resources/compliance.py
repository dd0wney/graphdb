...
class AsyncComplianceResource:
    ...
    async def audit_log(
        ...
        *,
        ...
        for name, val in (
            ...
            if val is not None:
                params[name] = val
        res = await self._t.request("GET", "/v1/compliance/audit-log", params=params)
        return _as_dict(res.data)

    async def get_masking_policy(self, tenant: str) -> dict[str, Any]:
    ...
    async def set_masking_policy(
        ...
        res = await self._t.request("POST", "/v1/compliance/masking-policy", json=body)
        return _as_dict(res.data)
