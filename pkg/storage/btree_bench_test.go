...
// Per-backend microbenchmarks for the basic Storage operations. The
// in-memory and BTree variants run the same workload against the same
// Storage interface; comparing them quantifies the durability cost.

func BenchmarkStorage_GetNode_Memory(b *testing.B) {
	dataDir, _ := os.MkdirTemp("", "bench-mem-getnode-*")
...
func BenchmarkStorage_GetNode_BTree(b *testing.B) {
	dataDir, _ := os.MkdirTemp("", "bench-btree-getnode-*")
	defer os.RemoveAll(dataDir)
...
func BenchmarkStorage_CreateNode_Memory(b *testing.B) {
	dataDir, _ := os.MkdirTemp("", "bench-mem-create-*")
	defer os.RemoveAll(dataDir)
...
func BenchmarkStorage_CreateNode_BTree(b *testing.B) {
	dataDir, _ := os.MkdirTemp("", "bench-btree-create-*")
	defer os.RemoveAll(dataDir)
...
func BenchmarkStorage_Traversal_Memory(b *testing.B) {
	dataDir, _ := os.MkdirTemp("", "bench-mem-trav-*")
	defer os.RemoveAll(dataDir)
...
func BenchmarkStorage_Traversal_BTree(b *testing.B) {
	dataDir, _ := os.MkdirTemp("", "bench-btree-trav-*")
	defer os.RemoveAll(dataDir)
...
// runTraversalBench builds a 100-node FOLLOWS chain and walks the first
// 10 hops per iteration. Takes Storage so both backends share the body.
