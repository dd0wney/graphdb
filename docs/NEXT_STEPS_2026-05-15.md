...
3. ~~**Pre-position: lift the HNSW insert out of `gs.mu`** (H2) and budget the auto-embed 2× (H3).~~ **✅ DONE 2026-06-02 (#266).** The ~140 µs HNSW insert now runs off `gs.mu`: decode + dimension-validate the node's vector properties under the lock (`planNodeVectorInserts`), run the expensive remove+add after release (`applyNodeVectorInserts`), on both create and update paths. The under-lock snapshot is required for **memory safety** (the off-lock insert must not read the live `node.Properties` a concurrent writer mutates), not just latency — proven by a `-race` test (8×25 concurrent `UpdateNode` on one vector node). Bad-dimension create now aborts as a true no-op (validated before publish); post-unlock insert errors are fail-soft (vector index is rebuilt by the app on restart). **H3** (auto-embed's 2 lock acquisitions / 2 fsyncs per node) is inherent to the create-then-embed-then-update flow — a documented sizing note, not a code fix.
4. **Index-structure + vector-read hygiene (M3/M4/M7, M5/M6)** — **clean portion ✅ DONE 2026-06-02; M3 + M7 remain (decision-laden, deferred).**
   - **M5 — ✅ #269.** `sync.Pool` for the HNSW per-layer visited set: 8× fewer bytes / 57% fewer allocs under concurrent search; behaviour-identical.
   - **M6 — ✅ #271.** Cache vector norms for cosine distance: 3.66× faster per distance call (980→268 ns), bit-identical (recall preserved via an exact-equality test).
   - **M4 — ✅ #272.** `findNewEntryPoint` now reads a per-level index (`nodesByLevel`) instead of scanning all nodes: O(N)→O(#levels). Bench ~5000× at 50k nodes (77 ns vs 390 µs).
   - **M3 — DEFERRED (snapshot-format-invasive).** Label-index removal is an O(K) scan run twice per `DeleteNode` under `gs.mu`. The audit's `map[uint64]struct{}` fix would change the on-disk snapshot value type of the *global* `nodesByLabel`/`edgesByType` (which are persisted), so it needs a **snapshot format version bump** — a deliberate decision, not a drop-in. A sorted-slice (binary-search removal) avoids the format change but keeps O(N) shift. Needs a design call before implementing.
   - **M7 — REFRAMED (NOT a dead-code delete).** The audit said "drop the dead global `nodesByLabel`/`edgesByType` mirror serving only dead code." **The code disagrees:** the global mirror has live tenant-blind readers — `FindNodesByLabel`/`FindEdgesByType` (`query_operations.go:145,215`), `node_adjacency.go:57` — and is snapshot-persisted (`persistence.go:65-66`). Removing it would break those callers + change the on-disk format. M7 is therefore "deprecate/migrate the tenant-blind `Find*` readers to per-tenant indexes, *then* drop the mirror," a multi-step reframe — not the one-line delete the audit implies. Needs a decision on whether the tenant-blind API is still wanted.
   - See `AUDIT_performance_saas_load_2026-06-02.md` § M3–M7.
