...
4. **Index-structure hygiene (M3/M4/M7) + vector-search read hygiene (M5/M6)** — **now the next critical-path step.** M3: label-index removal is an O(K) linear scan run twice per `DeleteNode` under `gs.mu` → `map[uint64]struct{}` or sorted slice. M4: `findNewEntryPoint` is an O(N) scan of the tenant's HNSW under `gs.mu` → O(1) max-layer candidate set. M7: drop the dead tenant-blind global `nodesByLabel`/`edgesByType` mirror (the per-tenant indexes now fully cover enumeration — confirm no live readers first). M5: `sync.Pool` the HNSW visited set. M6: cache query/stored norms. See `AUDIT_performance_saas_load_2026-06-02.md` § M3–M7.
