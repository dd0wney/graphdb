...
- **V1-spike — Deep-path traversal for the variable-length pattern path** (design spike; no implementation approved). `oit-cyber/interrogate` imports graphdb as a Go library in one process and must answer "which employees reach this path" over a POSIX directory tree joined with LDAP group data; it computes its own reachability closure today. Two capabilities in `pkg/query/match_path.go` would let that move into the engine:
  ...
  **Status: Spike.** Verified against `pkg/query/match_path.go` on `main` at `a07150e` (2026-08-31): `traverseVariablePath` and `getEdges` line numbers and behaviour above are read directly off that revision. No implementation is approved. Note for whoever picks this up: PR #546 reworks this same file (error returns, an engine-imposed-cap truncation signal), so re-read `match_path.go` before starting — the line numbers above shift once it lands.
