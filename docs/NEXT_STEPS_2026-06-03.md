...
**M-1 — Option C ✅ shipped (#393); Option A still the real fix (needs go-ahead).** Design doc `DESIGN_m1_wal_remanence_2026-06-10.md` (#391). **Option C** shipped: the missing **GDPR Art-17 Right-to-Erasure control** now exists in `pkg/compliance`, evaluated honestly (Partial + WAL-remanence note by default; flips to Compliant via the new `SystemInfo.ImmediateErasure` flag when A lands). No durability code touched. **Option A** = a `TruncateUpTo(lsn)` checkpoint (capture boundary LSN under the snapshot RLock, truncate only ≤ N) across all 3 WAL backends — the real fix, a multi-day durability-layer change that **needs an explicit go-ahead** before touching the WAL. Gates H-3; pairs with L-1/L-4.
