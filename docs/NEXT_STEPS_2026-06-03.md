...
**M-1 ✅ shipped 2026-06-12 (#396, Option A user-approved, all 3 backends)** — `wal.TruncateUpTo(lsn)` on plain/batched/compressed (+ `BatchedWAL.CheckpointLSN` boundary drain), `GraphStorage.CompactWAL()` (snapshot with boundary LSN captured under the snapshot RLock → truncate ≤ N), wired into `handleDeleteTenant` (loud-fail + retryable, like M-2). End-to-end remanence pin at the API layer. **Surfaced + fixed two pre-existing bugs:** (a) the **compressed WAL had zero crash durability** — single-op writes never logged (`enqueueWAL`/`writeToWALWithError` had no compressed branch) and `replayWAL` never read it back (the #288/#308 missing-branch class); (b) **`Snapshot()` was marshal-unsafe under live writers** (struct referenced live maps/nodes, marshal ran post-RUnlock — latent while snapshots only ran at `Close()`); all snapshot fields now deep-copied under the lock. Plus a `Transaction.Commit` barrier (`txWALBarrier`) closing the apply-in-memory-then-append window a checkpoint boundary could miss. L-1/L-4 remain accept-risk per the L-tier disposition.

**M-1 compliance control ✅ reworked 2026-06-12 (#393; #394 closed):** a prior session shipped an **Option-C interim** posture — a GDPR Art-17 (Right-to-Erasure) control in `pkg/compliance` reporting `StatusPartial` + an honest WAL-remanence note, gated on a new `SystemInfo.ImmediateErasure` flag. Option A (#396) made that interim factually stale, so #393 was **reworked before merge**: the control is now `StatusCompliant` unconditionally (tenant-delete purges the WAL synchronously via `CompactWAL` — structural, not deployment config), the never-wired `ImmediateErasure` flag is dropped, and a test pins the honesty guard (node/edge-level deletes still carry a WAL-remanence window until the next compaction, documented in the control's Notes). The companion planning note **#394 was closed as superseded**.

**Wave 3 design-gated remainder — ✅ ALL SHIPPED 2026-06-12:**
- **M-14** ✅ (#395) — `snapshot.json` magic-header (`GSNP`) + version + flags envelope (first use of the format-version discipline, v1); legacy headerless files keep loading. **Also fixed a latent restart-bricking bug:** encryption was wired via `SetEncryption` *after* `NewGraphStorage`, but the constructor loads the snapshot — an encrypted snapshot could never load at restart. `StorageConfig` gains `EncryptionEngine`/`KeyManager` (constructor-time wiring; new `DefaultStorageConfig` helper).
- **H-3** ✅ (#399) — WAL entry payloads sealed through the same engine (`GWE1` marker) at all four append dispatchers, unsealed in `replayEntry`. Legacy plaintext replays unchanged; enabling encryption over a plaintext-holding WAL triggers one constructor-time `CompactWAL` purge (the M-1 dependency, as predicted); ciphertext without an engine fails loud.
- **M-15 (OSS side)** ✅ (#397) — fail-closed SHA-256 `MANIFEST.sha256` verification before `plugin.Open`, absolute owner-only plugin dir (CWD-relative default resolved + deprecation-warned), `GRAPHDB_PLUGIN_ALLOW_UNVERIFIED=true` transition opt-out. **Cross-repo follow-up still open:** manifest-generation tooling in the graphdb-enterprise build.
- **M-7 follow-up** ✅ (#398) — `POST /api/users/{id}/revoke-tokens` admin endpoint over #390's `RevokeUserTokens`.

**Client release ✅ shipped 2026-06-12 (tags + GitHub releases only, user-decided; no PyPI/npm):** `python-sdk/v0.1.0` and `ts-client/v1.0.0` — both *first* tagged releases at their manifest versions, which dissolved the TS semver question (nothing previously released to break against). PyPI publish remains an open decision.

**TRACK S CLOSED 2026-06-12.** All 11 Highs + every executable Medium addressed; remaining tail is the M-15 enterprise-side manifest tooling (cross-repo) and the accept-risk L-tier dispositions.
...
6. **Current state (2026-06-12): TRACK S IS CLOSED.** Read § Track S. Waves 1 (#372–378) + 2 (#379/#380) + 3 (H-6 #385, H-5 #387, M-8 #383, M-2 #384, M-7 #390) shipped earlier; the design-gated remainder closed 2026-06-12: **M-14 #395, M-1 #396 (Option A, all 3 backends), M-15-OSS #397, M-7-followup #398, H-3 #399**, plus the client releases (`python-sdk/v0.1.0`, `ts-client/v1.0.0`). The track's only tail: M-15 enterprise-side manifest tooling (cross-repo) and the PyPI-publish decision. **No critical path is forced again** — next-track candidates per the standing queue: productization/operability (**first wave opened 2026-06-12 — see item 7**), the real-corpus coi-screen run (Milestone-1-proper, likeliest source of new evidence), GraphQL index-level pagination, the batched-WAL default sweep, CI hygiene (`cmd/...` test allowlist, gofmt lint gap). Don't re-commission a security audit without new evidence.

7. **Productization / operability wave ✅ FIRST PASS 2026-06-12 (#402/#403).** The dimension flagged in Decision 10 option C (and never previously opened) got its first wave — a documentation/release-hygiene pass, no functional code beyond display strings:
   - **#402** — finished the **#335 `cluso-graphdb` → `graphdb` rename in shipped artifacts**: goreleaser binaries (`cluso-server`/`cli`/`tui` → `graphdb-*`), Dockerfile build outputs + entrypoint + non-root image user, compose command, plus the `cmd/*` binary display strings (server log line, `cluso>` → `graphdb>` CLI prompt, TUI title, benchmark banners). **Breaking** for download scripts pinned to `cluso-*` asset names; takes effect at the next tagged release (README carries a transition note). Verified with a local `docker build`.
   - **#402** — **README factual-accuracy rewrite**: the documented quickstart was broken (every data-endpoint curl lacked auth, but all data routes are behind `requireAuth`) — new two-minute path includes `ADMIN_PASSWORD` + `/auth/login`; retired the ZeroMQ-replication sections (pkg/replication was removed in A8.1); fixed Go examples that imported the pre-#335 module path and nonexistent APIs; corrected the roadmap (FTS ships, ZeroMQ retired); added a Client Libraries table + a deployment-ordering note (create indexes before traffic).
   - **#402** — **customer-facing docs sweep**: stale `cluso-graphdb` import paths/URLs + fictional domains across 6 docs; `HA_QUICKSTART.md` replaced with an honest single-node status doc (its 3-node guide could not compile post-A8.1); `PRODUCTION_QUICKSTART.md` Step 3 fixed (wrong login path `/v1/auth/login` → `/auth/login`, wrong token field, wrong nodes path).
   - **#403** — **CLAUDE.md corrections**: `cmd/server` is the production server (not `cmd/graphdb`, which is a demo); the `-timeout 90s` package-test guidance under-buys `pkg/storage` (~120–170s since the Track S WAL work) — annotated with a 300s exception.
   - **Still open (the productization *decision*, not a doc bug):** README pricing tiers ($49 Pro / $299 Enterprise) were carried forward **unchanged** — pricing is the standing productization question from `CAPABILITIES_2026-05-10.md`; only the `[your-email]` contact placeholder was fixed. **Remaining wave candidates** (not yet done): single-node framing beyond the README note, FTS/LSA bootstrap-policy docs, the `cmd/...` CI test allowlist + gofmt lint gap.
