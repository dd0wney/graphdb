...
### Track A — Audit / tenancy isolation ✅ **CLOSED — original scope + A4-edges + A8.2 + A8.1 all done (A8.1 closed 2026-05-12 via Option B retirement)**
...
- **A8.1**: ✅ Done 2026-05-12 via Option B (delete). Spike at PR #127 (`docs/A8_1_SPIKE_2026-05-12.md`) reframed the directive's binary "rebuild vs delete" choice as a trilemma after surveying the library; `pkg/replication/` was forked-NNG/TCP fossilization with no salvageable foundation for a future `cmd/server`-native rebuild. Implementation: #129 (delete `pkg/admin/upgrade*` orphan), #130 (delete 4 legacy binaries), #133 (lift apply-gate to `pkg/wal/apply/`, delete `pkg/replication/`). Doc + observability cleanup: #134, #138, #139, #140 (in flight at the time of this update; not blocking the A8.1 closure since the structural work landed in #133).
- **A8.2**: **✅ Done 2026-05-10 (PR #81)** — removed route from both `cmd/graphdb-replica` (real leak via `GetAllNodesAcrossTenants()`) and `cmd/graphdb-nng-replica` (empty-stub, removed for symmetry). See A8.2 section below for the remove-vs-add-auth decision.
...
#### A8.1. Standalone replication binaries retired ✅ DONE 2026-05-12 via Option B (PRs #127, #129, #130, #133)

The spike (`docs/A8_1_SPIKE_2026-05-12.md`, PR #127) reframed the directive's binary "rebuild on cmd/server vs delete" as a trilemma after a real architectural read of `pkg/replication/`. Survey findings:

- `cmd/server` does not import `pkg/replication` — so "rebuild" was net-new multi-quarter work, not refactoring.
- `pkg/replication/` is two forked role implementations (TCP + NNG) plus an NNG-vocabulary socket abstraction, not a transport-agnostic core. ~9,150 LOC of fossilization that no `cmd/server`-native HTTP rebuild would reuse.
- `pkg/admin/upgrade*` (1,295 LOC) imported `pkg/replication` but had zero external callers — dead code at the call-graph level. Pre-existing orphan that was masking the architectural truth.
- The audit-load-bearing primitive was ~125 LOC: `WriteOperation` + `WriteExecutor` + `ApplyWriteOperation` fail-closed tenant gate, plus ~315 LOC of tests. Worth preserving regardless of the binaries' fate.

**Decision: Option B** — delete binaries + `pkg/admin/upgrade*` orphan + `pkg/replication/` library; lift the audit primitive to `pkg/wal/apply/`. Rationale in spike doc §6.

**Implementation:**
- PR #129 (merged): delete `pkg/admin/upgrade*.go` (-1,295 LOC, dead code at call-graph level).
- PR #130 (merged): delete `cmd/graphdb-{,nng-}{primary,replica}/` (-730 LOC; binaries had documented "use cmd/server" log-fatal gates).
- PR #133 (merged): lift apply-gate to `pkg/wal/apply/`, delete `pkg/replication/` (-9,284 LOC net + 3 third-party deps removed: `mangos/v3`, `Microsoft/go-winio`, `gorilla/websocket`).

**Total deletions: ~11,300 LOC + 3 third-party deps.** Snapshot version bump in `pkg/wal/apply` for the lifted primitive; A8 audit-regression row preserved with the new import path.

**Doc + observability cleanup:** #134, #138, #139, #140 (merged 2026-05-12). All documentation reconciled with the single-node deployment model.

**Acceptance met**: `GRAPHDB_LEGACY_BINARY` env var removed (no binaries reference it anymore). The horizontal-scale story is now honest: graphdb is single-node by design; future replication is a multi-quarter `cmd/server`-native rebuild if/when it's prioritized.

**A8 audit-coverage gap surfaced during the survey** (recorded as `coord-insight` `insight-1778554621-8abe61`, topic `audit-coverage-claims`): A8's closure narrative (`A8_REPLICATION_TENANCY_DESIGN.md` §Q6 acceptance) claimed all apply paths refused empty TenantID, but `replica_replication.go:121,136` was actually calling tenant-blind `storage.CreateNode`/`storage.CreateEdge`. Only mattered with `GRAPHDB_LEGACY_BINARY=1` on a TCP replica receiving WAL traffic — moot now that the code is deleted. The pattern (audit closure narrative may not cover every code path it claimed) is worth flagging for future multi-path audits.
...
#### S1. Storage interface extraction — spike with binary go/no-go ✅ **CLOSED 2026-05-12**
- [x] Design doc `docs/S1_STORAGE_INTERFACE_DESIGN.md`.
- [x] Interface Definition: `StorageReader`, `StorageWriter`, and `Storage` interfaces in `pkg/storage/interface.go`.
- [x] Full Migration: Algorithms, Query Engine, API, GraphQL, and CLI/Tools migrated to interfaces.
- [x] Verification: Compilation verified across the workspace; benchmarks show minimal overhead.
- **Decision**: Go for S1 was executed. Next step: S2 spike for persistent backends. ✅ **CLOSED 2026-05-12**
    - Implemented a custom B+Tree engine (`pkg/btree`) and `BTreeGraphStorage`.
    - Validated pluggability of the `Storage` interface.
    - Added `GRAPHDB_STORAGE_BACKEND` support to `cmd/server`.

#### S3. openCypher & Query Planner Architecture ✅ **CLOSED 2026-05-12**
- Implemented a formal Volcano physical execution model (`pkg/query/physical_plan.go`).
- Built a rule-based query planner with Index Seek optimization (`pkg/query/planner.go`).
- Extended parser to support `CALL ... YIELD` for graph algorithms.
- Unified query engine now leverages the pluggable `Storage` interface.

#### S4. Cypher Mutations (CREATE, SET, DELETE, REMOVE) ✅ **CLOSED 2026-05-12**
- Implemented `CreateOperator`, `SetOperator`, `DeleteOperator`, and `RemoveOperator` in the Volcano pipeline.
- Supported standalone and chained mutations.
- Handled `DETACH DELETE` for automatic relationship cleanup.
- Verified via E2E test suite.

#### S5. Cypher MERGE Operator ✅ **CLOSED 2026-05-12**
- Ported `MERGE` to the Volcano pipeline.
- Supported `ON CREATE SET` and `ON MATCH SET`.
- Verified node-only patterns with properties.

#### S7. OpenTelemetry (OTEL) Tracing Integration ✅ **CLOSED 2026-05-12**
- Implemented global `TracerProvider` with OTLP support (`pkg/telemetry`).
- Instrumented the entire HTTP API surface via `TraceMiddleware`.
- Instrumented the Query Engine (Executor, Planner, and Volcano Operators).
- Enabled request-level visibility from API entry to Physical Operator execution.

**Note on Query Engine Architecture**: The system now employs a dual-engine model. The new **Volcano Physical Engine** handles modern openCypher queries with optimized planning, while a **Legacy Fallback** mechanism preserves compatibility for features not yet ported (UNION, UNWIND, Aggregations).
...
H1 ✅ ──┐
        ├─→ A4 ✅ ──→ A4-edges ✅ ──→ A8.2 ✅ ──┐
H3 ✅ ──┘                                       ├─→ F1.1 ✅ ──┐
                                                └─→ H2 ─────── ├─→ F3 ✅ → A8.1 ✅ → S1 ✅ → S2 ✅ → S3 ✅ → S4 ✅ → S5 ✅ → S6 ✅ → S7 ✅ → **F4**
```

**Critical path**: ~~H1~~ → ~~A4~~ → ~~A4-edges~~ → ~~A8.2~~ → ~~F1.1~~ → ~~F3~~ → ~~A8.1~~ → ~~S1~~ → ~~S2~~ → ~~S3~~ → ~~S4~~ → ~~S5~~ → ~~S6~~ → ~~S7~~ → **F4 (Vector Isolation)**.
...
- **A8.1 closed via Option B** ✅ — the spike (#127) reframed the directive's binary "rebuild vs delete" as a trilemma after reading the library architecture; chose **delete + lift the audit primitive** because (a) `cmd/server` doesn't import `pkg/replication`, making "rebuild" net-new multi-quarter work; (b) `pkg/replication/` was forked NNG/TCP fossilization, not a salvageable foundation; (c) `pkg/admin/upgrade*` was dead code already. ~11,300 LOC + 3 third-party deps removed across PRs #129/#130/#133. S1 takes the head of the queue.
- **S1 now head** because A8.1's closure put graphdb back to a single-node-by-design architecture; the storage interface extraction is the next leverage point for both plugin extensibility and the future replication rebuild (if/when prioritized). S1's go/no-go is binary deliverable; its output reshapes the next planning checkpoint, not work for this one.
...
3. **A8.1's spike conclusion** — RESOLVED 2026-05-12. Spike (PR #127, `docs/A8_1_SPIKE_2026-05-12.md`) reframed as a trilemma after reading the library; picked **Option B (delete)** because `pkg/replication/` was NNG-shaped fossilization with no foundation for a future rebuild. Implementation via PRs #129/#130/#133.
...
- **~~Replication-binary deprecation (A8.1) may surface customer dependencies~~** — RESOLVED 2026-05-12 by Option B. The spike surfaced the contradiction (`PRODUCTION_QUICKSTART.md` documented the legacy binaries for production users while the A8 audit said they were unsafe) and forced honest single-node positioning; the doc rewrite in PR #138 is the user-facing closure. Customers running the deleted binaries with `GRAPHDB_LEGACY_BINARY=1` would have hit the breaking change — no evidence of any such users in the repo's commit history or issue tracker, but the spike doc's §7 Q2 flagged this risk explicitly.
