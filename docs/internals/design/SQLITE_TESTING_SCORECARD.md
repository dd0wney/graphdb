...
| 4 | Out-of-memory testing | ◐ | `pkg/alloc` + `alloctest` with BOTH loops — fail-once-at-N and fail-all-from-N — swept to termination (#484). Gates graphdb's own length-driven buffers; Go's implicit allocations are out of reach and the package says so. Found the LSN-reuse defect below. **Snapshot assembly gated (#507)**: `recordCursor.blob` is the per-property copy behind every node and edge decode, and `TestSnapshotReadPathUnderAllocationFailure` sweeps both loops to termination over the mmap read path. The safety property holds — under refusal a node may be unreadable, but nothing decodes WRONG and no half-built record escapes | LSM blocks and query results are still ungated. **New: a refused allocation is indistinguishable from a damaged record.** `getNode` documents "a damaged record reads as absent rather than crashing", which is right for damage and wrong for memory pressure — the node exists and we simply could not build it. Measured: 1 of 3 nodes vanish under fail-once, 2 of 3 under fail-all-from. Fixing it means giving the decode path a reason code, which changes an internal contract with 8 call sites |
