...
| 4 | Out-of-memory testing | ◐ | `pkg/alloc` + `alloctest` with BOTH loops — fail-once-at-N and fail-all-from-N — swept to termination (#484). Gates graphdb's own length-driven buffers; Go's implicit allocations are out of reach and the package says so. Found the LSN-reuse defect below. **Snapshot assembly gated (#507)**: `recordCursor.blob` is the per-property copy behind every node and edge decode, and `TestSnapshotReadPathUnderAllocationFailure` sweeps both loops to termination over the mmap read path. The safety property holds — under refusal a node may be unreadable, but nothing decodes WRONG and no half-built record escapes. **The read path now reports its refusals (PR B, `fix/storage-unreadable-single-record`)**: the six mmap loader helpers moved from a bare bool to `(T, error)` (PR A, #511), and PR B stops the single-record boundary collapsing that error to `ErrNodeNotFound`. `ErrRecordUnreadable` wraps either cause — `errRecordDamaged` (a record body that will not decode; the production case, since the snapshot CRC does not cover record bytes) or `alloc.ErrNoMemory` (a refused allocation) — so a caller can now tell "this ID does not exist" from "this ID exists and the store could not produce it". `TestSnapshotReadPathUnderAllocationFailure` was strengthened from logging the incompleteness it measured to asserting it: every ID the directory lists must now decode or come back `ErrRecordUnreadable` | LSM blocks and query results are still ungated. The multi-record read paths (`GetAllNodesForTenant`, the four `*PageForTenant` methods, `buildNodeListFromIDs`/`buildEdgeListFromIDs`, adjacency walks) still skip an unreadable record rather than reporting it — that is PR C, deferred because it changes `pkg/storage/interface.go`, which the parallel-agent rule says to propose rather than implement unilaterally. Two SQLite memory behaviours stay deliberately absent, so this row does not move to ✓: no retry-and-reclaim before refusal, and no soft heap limit (`sqlite3_soft_heap_limit64` equivalent) — a refusal from `alloc.Bytes` is final, recorded as a choice rather than a gap (planning doc, "Two SQLite memory behaviours graphdb deliberately does not have") |
| 5 | I/O error testing | ◐ | `pkg/vfs` + `vfstest.FaultFS` on the production path (#479); N-sweep (#481); `pkg/btree` and `pkg/lsm` migrated (#485), **`pkg/storage` and all three WAL flavours migrated (#507)** — a fault driver now reaches the snapshot, the load path and the WAL. **Now runs against background workers while a foreground reader runs** — `RoleFS` attributes each operation to an actor and `SweepRole` walks the fault through one actor's operations, checking that the actor's sequence is stable rather than assuming it (#492). Found the `WAL.Close` descriptor leak (#466), the LSM error-chain break (#485), and five defects in the LSM flush and compaction paths (#494), two of which were introduced by repairing the other three | `pkg/storage` (179 sites) not migrated = ADR 0002 stage 4. Its `syscall.Mmap` path does not fit `File` at all, and its 28 concurrent test files still inject nothing |
...
| 8 | Boundary-value testing | ◐ | Still no general `testcase()` equivalent, but the existence/ownership boundary now has one: the cross-principal equivalence sweep (PR C) asserts the observable is identical on both sides of it, for 8 operations, an intact store and a store with the target record damaged. `pkg/api` `TestCrossPrincipalEquivalence_IntactStore`, `TestCrossPrincipalEquivalence_UnderFault` | One boundary covered, not boundary values generically. No coverage-visible marker; a second boundary still needs the same manual case-by-case treatment |
...
| 11 | Malformed-database testing | ✓ | `FuzzMmapSnapshotCorruption` + `FuzzMmapSnapshotTruncation` (#477) + **`FuzzMmapSnapshotCRCRepaired` (#507)**, which mutates a byte and then RECOMPUTES the checksum so the file always passes the open guard. That is the case the other two structurally cannot reach: the CRC covers the directories, so a corrupt directory is caught at open and never meets a decoder. Removing that accident found an out-of-bounds read in `dirEntry` within seconds — and the first fix turned the panic into an unbounded loop, because `forEachEdgeID` takes its bound from `maxEdgeID`, which is file data. Both are fixed: `checkDirRange` refuses a header whose ID range exceeds its directory, and `dirEntry` bounds-checks as defence in depth. 9.0M executions clean afterwards | `TestCRCRepairedTargetReachesTheReadPaths` gates the TARGET at 146/200 inputs reaching the decoders. The truncation target manages 1 of 209, and that number is why this row was ◐ |
