...
| CC11-unreadable-not-missing | A record the snapshot directory lists but cannot decode returns `ErrRecordUnreadable`, never `ErrNodeNotFound` | all readers | `pkg/storage` `TestDamagedRecordIsNotReportedAsMissing`, `TestCheckInvariantsReportsADamagedRecord` | PR B (`fix/storage-unreadable-single-record`, not yet opened) |
