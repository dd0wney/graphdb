...
| CC15-truncated-query-serves-partial-result | A query that stops at an engine traversal limit returns 200, the rows that were found, and `X-Traversal-Truncated: true`; the header is absent when the traversal was complete | all REST consumers | `pkg/api` `TestTruncatedTraversalServesPartialResult`, `TestCompleteTraversalOmitsTheHeader` | #561 |
| CC16-claim-for-task-uniqueness | With the `claim_for_task`/`Claim` rule both required (`GRAPHDB_REQUIRED_UNIQUENESS_RULES`) and registered (`POST /admin/uniqueness-rules`), two REST `POST /nodes` creating `:Claim` with the same `for_task` give 201 then 409 | graphdb-coord | `pkg/api` `TestClaimForTaskUniqueness_RESTGivesCreatedThenConflict` | Task 9 / ADR 0001 |
