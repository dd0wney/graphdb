...
**Do not change either on-disk format without a version bump** — the snapshot file is customer-data-equivalent, and graphdb's format is load-bearing across external consumers. For the mmap path specifically, the correctness gate is a **JSON↔mmap public-interface equivalence oracle** (`fingerprintTenant` / `assertFingerprintEqual` in `pkg/storage/mmap_reopen_test.go`): an mmap-reopened store must enumerate byte-identically to the same store via JSON. Strengthen that oracle before relying on any new mmap optimization. (`CheckInvariants` — promoted out of the test binary in #468 — dispatches on representation. The shard path checks every derived structure. The mmap path (#474) rebuilds ground truth as `shard records ∪ (mmap base records − tombstones)`, shard winning, from raw records rather than the membership helpers — those fuse base and overlay and are the thing under test. **The two are not equivalent**: the mmap path covers per-tenant and per-label/per-type membership, the tenant list and edge endpoint integrity, but NOT the vector index or the adjacency lists, which have no mmap ground truth yet. `ErrInvariantsUnsupported` now means one thing only: a snapshot with no membership directory. This paragraph was corrected three times on 2026-08-28 (#471, #474 and this edit) — it tracks fast-moving code, so verify it against `pkg/storage/invariants.go` before relying on it.)
